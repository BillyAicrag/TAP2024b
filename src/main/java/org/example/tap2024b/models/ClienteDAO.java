...
public class ClienteDAO {
    ...
    public int INSERT() {
        int rowCount;
        String query = "INSERT INTO cliente(nomCte, telCte, emailCte)" +
    ...
    public void UPDATE(){
        String query = "UPDATE cliente SET nomCte = '" + this.nomCte + "', " +
    ...
    public void DELETE(){
        String query = "DELETE FROM cliente WHERE idCte = " + this.idCte;
        try {
            Statement stmt = Conexion.connection.createStatement();
            stmt.executeUpdate(query);
        }catch (Exception e ){
